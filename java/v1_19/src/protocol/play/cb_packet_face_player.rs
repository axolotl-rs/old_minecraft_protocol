use minecraft_protocol :: protocol :: PacketContent ; use minecraft_protocol :: protocol :: PacketSwitch ; use minecraft_protocol :: protocol :: Packet ; use std :: io :: { BufRead , Error , ErrorKind , Result , Write } ; use std :: str :: FromStr ;
 ...
 pub struct CbPacketFacePlayer ; impl Packet for CbPacketFacePlayer { type PacketIDType = i32 ; type PacketContent = PacketFacePlayerContent ; fn packet_id ( ) -> Self :: PacketIDType where Self : Sized { 55 } } pub struct PacketFacePlayerContent { pub feet_eyes: minecraft_protocol::data::VarInt ,
...
total_bytes += self.entity_feet_eyes.switch_write(false,writer)?;;
 ...
 Ok ( total_bytes ) } fn read < Reader : BufRead > ( reader : & mut Reader ) -> std :: io :: Result < Self > { let feet_eyes : minecraft_protocol::data::VarInt = PacketContent :: read ( reader ) ?;;
...
let entity_feet_eyes : PacketFacePlayerContentContent = PacketSwitch::switch_read(&isEntity,reader)?;;
 ...
 Switchtrue (minecraft_protocol::data::VarInt ) ,
