use minecraft_protocol :: protocol :: PacketContent ; use minecraft_protocol :: protocol :: PacketSwitch ; use minecraft_protocol :: protocol :: Packet ; use std :: io :: { BufRead , Error , ErrorKind , Result , Write } ; use std :: str :: FromStr ;
 ...
 pub struct CbPacketMap ; impl Packet for CbPacketMap { type PacketIDType = i32 ; type PacketContent = PacketMapContent ; fn packet_id ( ) -> Self :: PacketIDType where Self : Sized { 39 } } pub struct PacketMapContent { pub item_damage: minecraft_protocol::data::VarInt ,
...
total_bytes += self.data.switch_write(false,writer)?;;
 ...
 Ok ( total_bytes ) } fn read < Reader : BufRead > ( reader : & mut Reader ) -> std :: io :: Result < Self > { let item_damage : minecraft_protocol::data::VarInt = PacketContent :: read ( reader ) ?;;
