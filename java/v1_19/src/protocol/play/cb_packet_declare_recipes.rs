use minecraft_protocol :: protocol :: PacketContent ; use minecraft_protocol :: protocol :: PacketSwitch ; use minecraft_protocol :: protocol :: Packet ; use std :: io :: { BufRead , Error , ErrorKind , Result , Write } ; use std :: str :: FromStr ;
 ...
 } impl PacketContent for PacketDeclareRecipesContent { fn write < Writer : Write > ( self , writer : & mut Writer ) -> std :: io :: Result < usize > { let mut total_bytes = 0 ; total_bytes += self.recipes.write(writer)?;;

 Ok ( total_bytes ) } fn read < Reader : BufRead > ( reader : & mut Reader ) -> std :: io :: Result < Self > { let recipes : PacketDeclareRecipesContentArray = PacketContent :: read ( reader ) ?;;

 Ok ( Self { recipes } ) } } pub type PacketDeclareRecipesContentArray = Vec <PacketDeclareRecipesContentArrayContent >; pub struct PacketDeclareRecipesContentArrayContent { pub data_type: String ,

pub recipe_id: String ,

pub data: PacketDeclareRecipesContentArrayContentContent ,

 } impl PacketContent for PacketDeclareRecipesContentArrayContent { fn write < Writer : Write > ( self , writer : & mut Writer ) -> std :: io :: Result < usize > { let mut total_bytes = 0 ; total_bytes += self.data_type.write(writer)?;;

total_bytes += self.recipe_id.write(writer)?;;

total_bytes += self.data.switch_write(false,writer)?;;

 Ok ( total_bytes ) } fn read < Reader : BufRead > ( reader : & mut Reader ) -> std :: io :: Result < Self > { let data_type : String = PacketContent :: read ( reader ) ?;;
