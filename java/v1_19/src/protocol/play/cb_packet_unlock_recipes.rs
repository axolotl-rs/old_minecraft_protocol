mod cb_packet_unlock_recipes { use super ::*; use crate :: common :: protocol :: PacketContent ; use crate :: common :: protocol :: PacketSwitch ; use crate :: common :: protocol :: Packet ; use std :: io :: { BufRead , Error , ErrorKind , Result , Write } ; use std :: str :: FromStr ;

 pub struct CbPacketUnlockRecipes ; impl Packet for CbPacketUnlockRecipes { type PacketIDType = i32 ; type PacketContent = PacketUnlockRecipesContent ; fn packet_id ( ) -> Self :: PacketIDType where Self : Sized { 57 } } pub struct PacketUnlockRecipesContent { action: minecraft_data::common::data::VarInt ,
...
recipes_2: PacketUnlockRecipesContentContent ,
 ...
 } impl PacketContent for PacketUnlockRecipesContent { fn write < Writer : Write > ( self , writer : & mut Writer ) -> Result < usize > { let mut total_bytes = 0 ; total_bytes += self.action.write(writer)?;;
...
pub enum PacketUnlockRecipesContentContent { } impl PacketSwitch for PacketUnlockRecipesContentContent { type CompareType = minecraft_data::common::data::VarInt ; fn switch_read < Reader : BufRead > ( compare_to : & Self :: CompareType , reader : & mut Reader ) -> std :: io :: Result < Self > where Self : Sized { todo ! ( ) } fn switch_write < Writer : Write > ( self , write_compare : bool , writer : & mut Writer ) -> std :: io :: Result < usize > where Self : Sized { todo ! ( ) } }

 }

 pub use cb_packet_unlock_recipes ::*;
