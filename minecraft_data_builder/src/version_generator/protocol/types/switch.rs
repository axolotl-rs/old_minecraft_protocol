...
use crate::code_gen::{CompareTo, DataType, Field, GenerateType, InnerType, SwitchVariant, SwitchVariantType};
...
pub fn generate_top_level_switch(name: TypeName, compare_to: String, default: Option<String>, fields: HashMap<String, SwitchType>, state: &TypesGenerator) -> Result<GenerationResult, GenError> {
        ...
        if let SwitchType::Type(data) = data {
            ...
            match result {
                ...
                SubTypeResponse::CanNotBuild(build) => {
                    ...
                    variants.push(SwitchVariant::new(requirement.to_string(),
                                                     SwitchVariantType::Single(Field::new(&TypeName::Named("value".to_string()), DataType::default()))));
                }
                SubTypeResponse::AlreadyBuilt(already_build) => {
                    variants.push(SwitchVariant::new(requirement.to_string(),
                                                     SwitchVariantType::Single(Field::new(&TypeName::Named("value".to_string()), already_build.clone()))));
                }
                SubTypeResponse::SubType { data_type, generate } => {
                    match generate {
                        GenerateType::Container { children,fields, content_name } => {
                            variants.push(SwitchVariant::new(requirement.to_string(), SwitchVariantType::Container(fields)));
    ...
    let to = CompareTo::Specified { compare_to, compare_to_type: Box::new(state.get_data_type("varint").unwrap().clone()) };
    let data_type = DataType::new_generated_type(name.to_string(),
                                                 InnerType::Switch { compare_to: to.clone() });
...
pub fn generate_child_level_switch(parent_name: String, compare_to: CompareTo, default: Option<String>, fields: HashMap<String, SwitchType>, state: &TypesGenerator) -> Result<GenerationResult, GenError> {
    ...
    // For now assume all generic switches compare var_ints. We should change this in the future
    let data_type = DataType::new_generated_type(name.to_string(),
                                                 InnerType::Switch { compare_to: compare_to.clone() });
